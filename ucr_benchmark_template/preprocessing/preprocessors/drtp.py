...
def preprocess(name: str, 
    ...
    if out.exists():
        logger.info(f"[DRTP] Skipping `{name}` – already processed at {out}")
        return

    # Load splits
    logger.info(f"[DRTP] Loading raw data for `{name}`…")
    X_train = np.load(xtr)
    y_train = np.load(ytr)
    X_test = np.load(xts)
    y_test = np.load(yts)
    ...
    # Save
    logger.info(f"[DRTP] Saving processed data to {out}")
    np.savez_compressed(
        out,
        X_train=X_train_s,
        X_test=X_test_s,
